...
def showCategories():
    ...
    quotes = session.query(Quote).order_by(desc(Quote.created_on)).limit(10).all()
    return render_template('categories.html', categories=categories, quotes=quotes)
...
def showAllQuotes(category_name):
    ...
    return render_template('quotes.html', category=category, categories=categories, quotes=quotes, creator=creator)
...
def showQuote(category_name, quote_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicquote.html', quote=quote, category=category, creator=creator)
    else:
        return render_template('quote.html', category=category, quote=quote, creator=creator)
...
def newQuote(category_name):
    ...
    if request.method == 'POST':
        addNewQuote = Quote(author=request.form['author'], description=request.form['description'],
...
def editQuote(category_name, quote_id):
    ...
    if login_session['user_id'] != editedQuote.user_id:
        return "<script>function.myFunction() {alert('You are not authorized to edit this quote. " \
               "Add your favorite quotes and share them with the world. Those you will be able to edit.');}" \
    ...
    if request.method == 'POST':
        ...
        return render_template('editQuote.html', category_name=category_name, quote_id=quote_id, quote=editedQuote,
...
def deleteQuote(category_name, quote_id):
    ...
    if login_session['user_id'] != quoteToDelete.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete this quote. " \
               "Add your favorite quotes and share them with the world. Those you will be able to delete.');}" \
               "</script><body onload='myFunction()'>"
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteQuote.html', category_name=category_name, quote=quoteToDelete, creator=creator,
                               category=category)
